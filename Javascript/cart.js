...
function openModal() {
    ...
    let msg = 'Are you sure you want to order?'
    btnOrderNow.textContent = msg
    btnOrderNow.addEventListener('click', () => {
        modal.style.display = 'flex';
        modal.style.animation = 'fadeIn .4s ease-in';
        mobileDown.style.animation = 'mobileDown .2s ease-in'
        btnOrderNow.textContent = 'Order';
        mobileDown.style.visibility = 'hidden'
    })
    modal.addEventListener('click', (e) => {
        if (e.target === modal) {
            modal.style.display = 'none';
            mobileDown.style.animation = 'mobileUp .4s ease-in'
            mobileDown.style.display = 'block'
            mobileDown.style.visibility = 'visible'
        }
    })
...
const btnOrderNow = document.querySelector('#orderNow')
btnOrderNow.addEventListener('click', openModal)

const specialCodes = ['Riot', 'Enzo', 'Enis', 'Daddy', 'InnovationAcademy', 'Zoja', 'Nusja'];
